...
def main() -> int:
    ...
    parser.add_argument(
        "--emit-luraph-profile",
        action="store_true",
        help="Print a Luraph146Like-style config snippet using sample-derived signature/prefix",
    )
    ...
    if style_file is not None:
        ...
        print('  }')
        if args.emit_luraph_profile:
            print("\nLuraph146Like profile snippet:")
            print("return {")
            print('  ObfuscationProfile = "Luraph146Like",')
            print("  LuaVersion = \"LuaU\",")
            print("  UniqueOutput = true,")
            print(f'  StyleSignature = "{signature}",')
            print("  StyleProfile = {")
            print(f'    VarNamePrefix = "{prefix}",')
