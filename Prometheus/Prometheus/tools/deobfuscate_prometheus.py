...
def compute_style_metrics(lua_source: str) -> dict[str, float]:
    assignments = ASSIGNMENT_RE.findall(lua_source)
    assignment_count = len(assignments)
    literals = [raw for _, _, raw in assignments]
    literal_bytes = sum(len(raw) for raw in literals)
    total_bytes = max(1, len(lua_source))
    identifier_count = len(re.findall(r"\b[A-Za-z_][A-Za-z0-9_]*\b", lua_source))

    return {
        "assignment_count": float(assignment_count),
        "literal_bytes_ratio": literal_bytes / total_bytes,
        "identifier_density": identifier_count / max(1.0, total_bytes / 1000.0),
        "has_vmify_tokens": 1.0
        if re.search(r"\b(vm|opcode|dispatch|execute|instruction)\b", lua_source, flags=re.IGNORECASE)
        else 0.0,
    }


def suggest_profile_from_metrics(metrics: dict[str, float]) -> str:
    # Strong constant-table footprint or VM token hints => RedzHub-like profile.
    if (
        metrics["assignment_count"] >= 120
        or metrics["literal_bytes_ratio"] >= 0.18
        or metrics["has_vmify_tokens"] >= 1.0
    ):
        return "RedzHubLikeVM"
    return "RobloxSafeVMLevel3"
...
def main() -> int:
    ...
    if style_file is not None:
        ...
        metrics = compute_style_metrics(style_source)
        profile = suggest_profile_from_metrics(metrics)
        print("\nSuggested pipeline options:")
        print(f'  Profile = "{profile}"')
        print("  UniqueOutput = true")
        print("  OutputPaddingRatio = 0.98")
        print(f'  StyleSignature = "{signature}"')
        print('  StyleProfile = {')
        print(f'    VarNamePrefix = "{prefix}",')
        print('  }')
        print("\nSample metrics:")
        print(f"  assignment_count = {int(metrics['assignment_count'])}")
        print(f"  literal_bytes_ratio = {metrics['literal_bytes_ratio']:.3f}")
        print(f"  identifier_density = {metrics['identifier_density']:.3f}")
        print(f"  has_vmify_tokens = {int(metrics['has_vmify_tokens'])}")
