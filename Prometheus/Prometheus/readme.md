...
```
Then copy the suggested profile + style options into your pipeline config.
If the sample looks RedzHub-style (string-table + VM structure), the helper now auto-suggests `Profile = "RedzHubLikeVM"`.
...
### Roblox-safe VM profile (level 3)

To force virtualization even for Roblox-safe builds, use:

```lua
Profile = "RobloxSafeVMLevel3"
```

This profile is designed for LuaU/Roblox compatibility and includes:
- `Vmify` enabled by default,
- conservative number-expression settings for stability,
- optional output-size padding target set to `0.98` (`OutputPaddingRatio = 0.98`) to keep output density near your requested level with zero runtime overhead (padding is added as a block comment).
...
return {
  ...
  Steps = {
    ...
    { Name = "NumbersToExpressions", Settings = { NumberRepresentationMutaton = false } },
    { Name = "Vmify", Settings = {} },
...
### RedzHub-like VM profile

If your reference output is a dense string-table virtualization style, use:

```lua
Profile = "RedzHubLikeVM"
```

This profile keeps LuaU-safe behavior while combining:
- encrypted strings,
- constant-array string indirection,
- split-string pressure,
- VM wrapper,
- output padding ratio target `0.98`.
...
## Luraph-like fast profile

If you want a stronger Luraph-inspired setup with controlled runtime overhead, use:

```lua
Profile = "LuraphLikeFast"
```

You can still override `Steps` manually after profile application if needed.
